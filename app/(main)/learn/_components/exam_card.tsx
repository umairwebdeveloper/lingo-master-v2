...
const ExamCard: React.FC<ExamCardProps> = ({ exam }) => {
    ...
    const handleReview = () => {
        router.push(
            `/learn/${exam.id}/complete?total=${exam.questionsCount}&correct=${exam.correctCount}&wrong=${exam.incorrectCount}`
        );
    };
        ...
        >
            <div className="flex gap-3 justify-between">
                <div className="mb-2 sm:mb-0">
                    <h3
                        className={`${albertSans.className} font-semibold text-[#3D3D3D] text-[18px] font-medium leading-normal`}
                    >
                        {exam.name}
                    </h3>
                </div>
                        ...
                        {exam.questionsCount === 0 ? (
                            ...
                            <>
                                <div className="inline-flex text-xs text-gray-600 text-wrap items-center space-x-1 bg-[#F6F6F6] border border-[#E7E7E7] rounded-lg px-2 py-1">
                                    ...
                                    />
                                    <span
                                        className={`${albertSans.className} text-[#3D3D3D] text-[13px] font-medium`}
                                    >
                                        {exam.questionsCount}
                                    </span>
                                </div>
      ...
      `}
                ...
                {exam.questionsCount === 0 ? (
                    ...
                    <div className="flex items-center w-full space-x-2">
                        ...
                        <span
                            className={`${albertSans.className} text-[14px] font-medium leading-[150%] tracking-[-0.21px] text-[var(--GreyScale-900,#3D3D3D)]`}
                        >
                            {exam.progress}%
                        </span>
                    </div>
